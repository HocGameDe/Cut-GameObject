...
public class Cutter : MonoBehaviour
{
    ...
    private void SplitPolygon(List<Vector2> path)
    {
        ...
        void InsertCutPointIntoPoints(List<Vector2> pathOrigin, List<Vector2> pointsSplit, Vector2 pointBeginCollision, Vector2 pointEndCollision)
        {
            ...
            Vector2? pointInsertFirst = CalculatorPoints.GetIntersectionOfLines(pathOrigin[indexPointFirst]
                                                                                , pathOrigin[indexPointFirst == 0 ? pathOrigin.Count - 1 : indexPointFirst - 1]
                                                                                , pointBeginCollision, pointEndCollision);
            Vector2? pointInsertLast = CalculatorPoints.GetIntersectionOfLines(pathOrigin[indexPointLast]
                                                                                , pathOrigin[indexPointLast == pathOrigin.Count - 1 ? 0 : indexPointLast + 1]
                                                                                , pointBeginCollision, pointEndCollision);
